...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nombre) || empty($password)) {
        ...
        // Conexión a base de datos de baloncesto
        $pdo = getConnection('club_baloncesto');

        // Comprobar credenciales
        $stmt = $pdo->prepare('SELECT id, nombre, rol, contrasena FROM usuarios_baloncesto WHERE nombre = ?');
        $stmt->execute([$nombre]);
        $user = $stmt->fetch();
...
<head>
    ...
    <title>Login - Club Baloncesto</title>
    <style>
        html, body {
  margin: 0;
  padding: 0;
