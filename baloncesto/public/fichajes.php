...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($canAdmin && isset($_POST['action_sug'])) {
        if ($_POST['action_sug'] === 'create') {
            $stmt = $pdo->prepare(
                "INSERT INTO sugerencias_fichajes_baloncesto (entrenador_id, descripcion, posicion)
                 VALUES (?, ?, ?)"
            );
            $stmt->execute([$userId, trim($_POST['descripcion']), $_POST['posicion']]);
        } elseif ($_POST['action_sug'] === 'edit') {
            $stmt = $pdo->prepare(
                "UPDATE sugerencias_fichajes_baloncesto
                    SET descripcion = ?, posicion = ?
                  WHERE id = ?"
            );
            $stmt->execute([trim($_POST['descripcion']), $_POST['posicion'], (int)$_POST['sug_id']]);
        } elseif ($_POST['action_sug'] === 'delete') {
            $stmt = $pdo->prepare("DELETE FROM sugerencias_fichajes_baloncesto WHERE id = ?");
            $stmt->execute([(int)$_POST['sug_id']]);
        }
    }
    if ($canScout && isset($_POST['action_fichar'], $_POST['libre_id'])) {
        $stmt = $pdo->prepare("SELECT * FROM jugadores_libres_baloncesto WHERE id = ?");
        ...
        if ($libre) {
            ...
            $del = $pdo->prepare("DELETE FROM jugadores_libres_baloncesto WHERE id = ?");
...
$sugs = $pdo->query(
    "SELECT s.id, s.descripcion, s.posicion, u.nombre AS entrenador
       FROM sugerencias_fichajes_baloncesto s
       JOIN usuarios_baloncesto u ON s.entrenador_id = u.id
    ORDER BY s.id DESC"
)->fetchAll();
...
if ($canView) {
    $libres = $pdo->query(
        "SELECT * FROM jugadores_libres_baloncesto ORDER BY nombre"
    )->fetchAll();
}
?>
<!DOCTYPE html>
...
<title>Fichajes - Club Baloncesto</title>
<style>
body {
  background: #111;
  color: #fff;
...
.right-panel h2, .right-panel .table-header {
  flex-shrink: 0;
  padding: 15px 20px;
  border-bottom: 1px solid #333;
}
  ...
  <div class="container">
    ...
    <div class="left-panel">
      ...
      <div class="card-container">
        ...
        <div class="card"
             data-id="<?= $s['id'] ?>"
             data-desc="<?= htmlspecialchars($s['descripcion'], ENT_QUOTES) ?>"
             data-pos="<?= $s['posicion'] ?>">
            ...
            <tr>
              ...
              <td>
                <form method="post" style="margin:0;">
                  <input type="hidden" name="action_fichar" value="1">
                  <input type="hidden" name="libre_id" value="<?= $l['id'] ?>">
                  <button class="btn-save" type="submit">Fichar</button>
                </form>
              </td>
              <?php endif; ?>
            </tr>
...
function toggleForm() {
  document.getElementById('sugForm').classList.toggle('hidden');
}
document.querySelectorAll('.panel-header .btn-save').forEach(btn =>
