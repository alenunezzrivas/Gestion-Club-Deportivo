...
if ($canConvocar && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (count($selected) === 12) {
        ...
        $stmt = $pdo->prepare("INSERT INTO convocatorias_baloncesto (fecha, descripcion) VALUES (CURDATE(), ?)");
        $stmt->execute([trim($_POST['descripcion'] ?? 'Convocatoria')]);
        $convId = $pdo->lastInsertId();
        $ins = $pdo->prepare("INSERT INTO convocatoria_jugadores_baloncesto (convocatoria_id, jugador_id) VALUES (?, ?)");
        ...
        $error = "Selecciona exactamente 5 jugadores. Has seleccionado " . count($selected);
    }
}
...
$conv = $pdo->query("SELECT id, descripcion, fecha FROM convocatorias_baloncesto ORDER BY id DESC LIMIT 1")->fetch();
$selectedIds = [];
if ($conv) {
    $sel = $pdo->prepare("SELECT jugador_id FROM convocatoria_jugadores_baloncesto WHERE convocatoria_id=?");
    $sel->execute([$conv['id']]);
...
<head>
  ...
  <title>Convocatorias - Club Baloncesto</title>
  <style>
    html, body {
      margin: 0;
      padding: 0;
  ...
  <div class="container">
    ...
    <div class="left-panel">
      <div class="panel-header">
        <h2>Plantilla</h2>
        <div class="counter">Seleccionados: <span id="count"><?= count($selectedIds) ?></span>/12</div>
      </div>
    ...
    <div class="right-panel">
      ...
      <table class="table">
        ...
        <?php
          $order = ['Base','Escolta','Alero','Ala-pívot','Pívot'];
          $rows = [];
