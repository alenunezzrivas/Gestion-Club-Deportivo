...
if (!$isTrainer && $userRole === 'jugador') {
    $stmt = $pdo->prepare("SELECT posicion FROM jugadores_baloncesto WHERE usuario_id = ?");
    $stmt->execute([$userId]);
...
if ($isTrainer && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        $stmt = $pdo->prepare(
            "INSERT INTO tareas_baloncesto (titulo, descripcion, position_group)
    ...
    if ($action === 'delete' && !empty($_POST['task_id'])) {
        $stmt = $pdo->prepare("DELETE FROM tareas_baloncesto WHERE id = ?");
        $stmt->execute([intval($_POST['task_id'])]);
    }
    header('Location: tareas_basket.php');
    exit;
}

// 6) Recuperar todas las tareas
$tareas = $pdo->query("SELECT * FROM tareas_baloncesto ORDER BY id DESC")->fetchAll();
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
<title>Tareas Baloncesto - Club Deportivo</title>
<style>
  html, body {
